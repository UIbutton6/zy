    ...
    GetPermissionList({ commit }) {
      return new Promise((resolve, reject) => {
        ...
        queryPermissionsByUser(params).then(response => {
          // response.result.menu = menus; // set
          const { menu, auth, allAuth } = response.result;
          sessionStorage.setItem(USER_AUTH,JSON.stringify(auth));
          sessionStorage.setItem(SYS_BUTTON_AUTH,JSON.stringify(allAuth));
          if (menu && menu.length > 0) {
            //update--begin--autor:qinfeng-----date:20200109------for：JEECG-63 一级菜单的子菜单全部是隐藏路由，则一级菜单不显示------
            menu.forEach(item => {
              if (item["children"]) {
                let hasChildrenMenu = item["children"].filter(i => !i.hidden || i.hidden == false )
                if (hasChildrenMenu == null || hasChildrenMenu.length == 0) {
                  item["hidden"] = true
                }
              }
            })
            // console.log(" menu show json ", menu)
